  ...
  Router.prototype.route = function () {
    ...
    try {
      for (var _b = __values(this.routeTable), _c = _b.next(); !_c.done; _c = _b.next()) {
        var RouteInfo_1 = _c.value;

        if (routePath.indexOf(RouteInfo_1.path) >= 0) {
...
var NewsFeedView = function (_super) {
  ...
  function NewsFeedView(containerId) {
    ...
    _this.api = new api_1.NewsFeedApi(config_1.NEWS_URL); // JSON -> Object  
    ...
    return _this; // window.store.totalPage = this.feeds.length / 10;
  }
...
window.store = store; // 백틱 : 문자열로 만들 수 있음
// 브라킷 대괄호
// 브레이스 {} 중괄호
